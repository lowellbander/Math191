...
Let $\{(x_i, y_i) | i = 1, N\}$ be the training sample. Fix some hypothesis(?) $h(x)$ [e.g. $h(x) = ax + b$] such that we minimize the squared error $\sum_{i=1}^{N} (y_i - h(x_i))^2$. \\

We observe(?) a new test point coming in $(x_o, y_o)$ such that they(?) are distributed by a probability density function $p$.\\
